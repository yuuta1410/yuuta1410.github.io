...
function projectFrom(value: unknown): Project {
  ...
  if (!titleEn || !titleVi)
    throw new Error('English and Vietnamese titles are required');
...
function socialsFrom(value: unknown): SocialLink[] {
  ...
  const socials = value.slice(0, 30).map((item, index) => {
    ...
    if (!platform || !label)
      throw new Error('Every social link needs a platform and label');
    const urlError = socialLinkError(platform, url);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === 'save-settings')
      await saveSettings(settingsFrom(body.settings));
    else if (action === 'save-project')
      await upsertProject(projectFrom(body.project));
    else if (action === 'delete-project') {
      const id = text(body.id, 100);
      if (!id) throw new Error('Project id is required');
      await deleteProject(id);
    } else if (action === 'save-socials')
      await saveSocialLinks(socialsFrom(body.socials));
    else throw new Error('Unknown action');
    return Response.json({
      ok: true,
      content: await getPortfolioContent(true),
    });
  } catch (error) {
    const message =
